...
export const GearManagementPanel: React.FC<GearManagementPanelProps> = ({ onRefreshChain, onClose, initialSelectedGuid, exportDebugData }) => {
  ...
  const [valueMapText, setValueMapText] = useState<string>('');
  const [reverseValueMapText, setReverseValueMapText] = useState<string>('');
  const [valueMapError, setValueMapError] = useState<string | null>(null);
  const [reverseValueMapError, setReverseValueMapError] = useState<string | null>(null);
  const [optionRows, setOptionRows] = useState<ParameterOptionRow[]>([]);
  const [lockFormulaDesc, setLockFormulaDesc] = useState<boolean>(false);

  // Parameter level alias editing state
  const [newParamAliasInput, setNewParamAliasInput] = useState<string>('');
  const [editingAliasIdx, setEditingAliasIdx] = useState<number | null>(null);
  const [editingAliasVal, setEditingAliasVal] = useState<string>('');
  const [bulkParamAliasText, setBulkParamAliasText] = useState<string>('');
  ...
  // Migration assistant and helpers for parameter options
  const migrateToOptionRows = (param: GearProfileParameter): ParameterOptionRow[] => {
    if (param.optionRows && param.optionRows.length > 0) {
      return param.optionRows;
    }
    
    const rows: ParameterOptionRow[] = [];
    let mapObj: Record<string, any> = {};

    if (param.valueMap) {
      mapObj = param.valueMap;
    } else if (param.valueMapJson) {
      try {
        mapObj = JSON.parse(param.valueMapJson);
      } catch (e) {
        // ignore
      }
    }
    ...
    if (mapObj && typeof mapObj === 'object') {
      Object.entries(mapObj).forEach(([displayLabel, exportValue]) => {
        const existingRow = rows.find(r => String(r.exportValue) === String(exportValue));
        if (existingRow) {
          if (!existingRow.aliases.includes(displayLabel)) {
            existingRow.aliases.push(displayLabel);
          }
        } else {
          rows.push({
            ...
            displayLabel,
            aliases: [],
            exportValue: exportValue as string | number,
            isDefault: (param.defaultExportValue !== undefined && String(param.defaultExportValue) === String(exportValue)) ||
                       (param.defaultValue !== undefined && String(param.defaultValue) === String(exportValue)),
            notes: '',
  ...
  const handleAddOptionRow = () => {
    setOptionRows([
      ...optionRows,
      {
        ...
        isDefault: false,
        notes: '',
        sortOrder: optionRows.length
      }
    ]);
  };

  const handleUpdateOptionRow = (index: number, key: keyof ParameterOptionRow, value: any) => {
    const updated = [...optionRows];
    updated[index] = {
      ...updated[index],
      [key]: value
    } as ParameterOptionRow;
    setOptionRows(updated);
  };

  const handleSetDefaultRow = (index: number, checked: boolean) => {
    const updated = optionRows.map((row, idx) => ({
      ...row,
      isDefault: idx === index ? checked : false
    }));
    setOptionRows(updated);
    
    if (checked && paramForm) {
      setParamForm({
        ...paramForm,
        defaultValue: optionRows[index].exportValue,
        defaultExportValue: optionRows[index].exportValue
      });
    }
  };
  ...
  const handleSyncFromRawJSON = () => {
    try {
      let parsedValueMap: Record<string, any> = {};
      if (valueMapText && valueMapText.trim() !== '') {
        parsedValueMap = JSON.parse(valueMapText);
      }
      
      const newRows: ParameterOptionRow[] = [];
      Object.entries(parsedValueMap).forEach(([displayLabel, exportValue]) => {
        const existingRow = newRows.find(r => String(r.exportValue) === String(exportValue));
        if (existingRow) {
          if (!existingRow.aliases.includes(displayLabel)) {
            existingRow.aliases.push(displayLabel);
          }
      ...
      setValueMapError("Failed to import from JSON: " + e.message);
    }
  };

  // Helper for parameter alias management inside parameter editor drawer
  const getParamSavedAliases = (param: GearProfileParameter): string[] => {
    return param.savedAliases && param.savedAliases.length > 0 ? param.savedAliases : (param.aliases || []);
  };

  const handleAddParamSavedAlias = (aliasToAdd: string) => {
    if (!paramForm || !aliasToAdd.trim()) return;
    const clean = aliasToAdd.trim();
    const alphanumericOnly = clean.replace(/[^a-zA-Z0-9]/g, '');
    if (!alphanumericOnly) {
      setParamAliasError("Alias cannot be empty or consist solely of punctuation.");
      return;
    }

    const currentSaved = getParamSavedAliases(paramForm);
    if (currentSaved.some(a => a.toLowerCase() === clean.toLowerCase())) {
      setParamAliasError(`Alias "${clean}" already exists in Saved Aliases.`);
      return;
    }

    const updatedSaved = [...currentSaved, clean];
    setParamForm({
      ...paramForm,
      aliases: updatedSaved,
      savedAliases: updatedSaved
    });
    setNewParamAliasInput('');
    setParamAliasError(null);
  };

  const handleUpdateParamSavedAlias = (index: number, updatedVal: string) => {
    if (!paramForm) return;
    const clean = updatedVal.trim();
    if (!clean.replace(/[^a-zA-Z0-9]/g, '')) {
      setParamAliasError("Alias cannot be empty or consist solely of punctuation.");
      return;
    }

    const currentSaved = [...getParamSavedAliases(paramForm)];
    currentSaved[index] = clean;
    setParamForm({
      ...paramForm,
      aliases: currentSaved,
      savedAliases: currentSaved
    });
  ...
  const handleRemoveParamSavedAlias = (index: number) => {
    if (!paramForm) return;
    const currentSaved = [...getParamSavedAliases(paramForm)];
  ...
  const handleApplyBulkParamAliases = () => {
    if (!paramForm || !bulkParamAliasText.trim()) return;
    const tokens = bulkParamAliasText.split(/[\n,;]+/);
    const currentSaved = getParamSavedAliases(paramForm);
    const existingSet = new Set(currentSaved.map(a => a.toLowerCase().trim()));
    const added: string[] = [];

    for (const rawToken of tokens) {
      const clean = rawToken.trim();
      if (!clean) continue;
      if (!clean.replace(/[^a-zA-Z0-9]/g, '')) continue;
    ...
    if (added.length === 0) {
      setParamAliasError("No new valid aliases found in bulk input.");
      return;
    }

    const nextSaved = [...currentSaved, ...added];
    setParamForm({
      ...paramForm,
      aliases: nextSaved,
      savedAliases: nextSaved
    });
  ...
  const handleDeduplicateParamAliases = () => {
    if (!paramForm) return;
    const currentSaved = getParamSavedAliases(paramForm);
    const seen = new Set<string>();
    const deduped: string[] = [];
    for (const a of currentSaved) {
    ...
    setParamForm({
      ...paramForm,
      aliases: deduped,
      savedAliases: deduped
    });
    setParamAliasError(null);
  };

  const handleNormalizeParamAliases = () => {
    if (!paramForm) return;
    const currentSaved = getParamSavedAliases(paramForm);
    const seen = new Set<string>();
    const normalized: string[] = [];
    for (const a of currentSaved) {
      const norm = a.replace(/[^a-zA-Z0-9 _-]/g, '').trim();
  ...
  const checkParamAliasCollisions = (
    currentParam: GearProfileParameter,
    profile: GearProfile | null,
    currentParamIdx: number | null
  ) => {
    if (!profile || !currentParam) return [];
    const saved = getParamSavedAliases(currentParam);
    const collisions: { alias: string; otherParamName: string }[] = [];
    if (saved.length === 0) return collisions;

    profile.parameters.forEach((p, idx) => {
      if (currentParamIdx !== null && idx === currentParamIdx) return;
      const otherAliases = [
        p.displayName,
        p.canonicalName,
        p.export?.name,
      ...
      saved.forEach(myAlias => {
        const normMyAlias = myAlias.toLowerCase().trim();
        if (normMyAlias && otherAliases.includes(normMyAlias)) {
          if (!collisions.some(c => c.alias === myAlias && c.otherParamName === p.displayName)) {
            collisions.push({ alias: myAlias, otherParamName: p.displayName });
          }
        }
      });
    });
  ...
  const handleStartEditParam = (index: number) => {
    ...
    if (rawParam) {
      const saved = rawParam.savedAliases || rawParam.aliases || [];
      const autoGen = rawParam.autoGeneratedAliases && rawParam.autoGeneratedAliases.length > 0
        ? rawParam.autoGeneratedAliases
        : generateAliasesForXmlParam(rawParam.export?.name || rawParam.displayName || rawParam.canonicalName || '');
      const rawMap = rawParam.rawMappingAliases || [];
      const effective = Array.from(new Set([
        ...saved,
        ...rawMap,
        ...autoGen,
        rawParam.displayName,
        rawParam.canonicalName,
        rawParam.export?.name
      ].filter(Boolean)));
      ...
      setParamForm({
        ...
        savedAliases: saved,
        aliases: saved,
        rawMappingAliases: rawMap,
        autoGeneratedAliases: autoGen,
        effectiveAliases: effective,
        visual: {
      ...
      setValueMapText(rawParam.valueMap ? JSON.stringify(rawParam.valueMap, null, 2) : '');
      setReverseValueMapText(rawParam.reverseValueMap ? JSON.stringify(rawParam.reverseValueMap, null, 2) : '');
      setValueMapError(null);
      setReverseValueMapError(null);
      
      const migrated = migrateToOptionRows(rawParam);
      setOptionRows(migrated);
      setLockFormulaDesc(!!rawParam.conversion?.formula);
  ...
  const handleSaveParamEdit = async () => {
    ...
    const invalidOption = optionRows.some(row => !row.displayLabel.trim() || !String(row.exportValue).trim());
    if (invalidOption) {
      setValueMapError("Please fill out both Display Label and Export Value for all option rows.");
      return;
    }

    // 1. Generate from optionRows if they exist
    const generatedValueMap: Record<string, string | number> = {};
    const generatedReverseValueMap: Record<string, string> = {};

    optionRows.forEach((row) => {
      if (!row.displayLabel || row.displayLabel.trim() === '') return;
      const parsedExportVal = parseFloat(String(row.exportValue));
      const finalExportVal = isNaN(parsedExportVal) ? row.exportValue : parsedExportVal;
      
      generatedValueMap[row.displayLabel.trim()] = finalExportVal;
      generatedReverseValueMap[String(finalExportVal)] = row.displayLabel.trim();

      if (row.aliases && Array.isArray(row.aliases)) {
        row.aliases.forEach((alias) => {
          if (alias && alias.trim()) {
            generatedValueMap[alias.trim()] = finalExportVal;
          }
        });
      }
    });

    const valMapJsonStr = Object.keys(generatedValueMap).length > 0 ? JSON.stringify(generatedValueMap, null, 2) : '';
    const revValMapJsonStr = Object.keys(generatedReverseValueMap).length > 0 ? JSON.stringify(generatedReverseValueMap, null, 2) : '';

    // 2. Fall back to manual JSON if optionRows is empty
    let parsedValueMap = Object.keys(generatedValueMap).length > 0 ? generatedValueMap : undefined;
    let valMapStr = valMapJsonStr || undefined;
    if (!parsedValueMap && valueMapText && valueMapText.trim() !== '') {
      try {
        parsedValueMap = JSON.parse(valueMapText);
        valMapStr = valueMapText;
        setValueMapError(null);
      } catch (e: any) {
        setValueMapError("Invalid JSON in Value Map: " + e.message);
        return;
      }
    }

    let parsedReverseValueMap = Object.keys(generatedReverseValueMap).length > 0 ? generatedReverseValueMap : undefined;
    let revValMapStr = revValMapJsonStr || undefined;
    if (!parsedReverseValueMap && reverseValueMapText && reverseValueMapText.trim() !== '') {
      try {
        parsedReverseValueMap = JSON.parse(reverseValueMapText);
        revValMapStr = reverseValueMapText;
        setReverseValueMapError(null);
      } catch (e: any) {
        setReverseValueMapError("Invalid JSON in Reverse Value Map: " + e.message);
        return;
      }
    }
    ...
    const autoGenAliases = paramForm.autoGeneratedAliases && paramForm.autoGeneratedAliases.length > 0
      ...
      : generateAliasesForXmlParam(paramForm.export?.name || paramForm.displayName || paramForm.canonicalName || '');
    ...
    const finalForm: GearProfileParameter = {
      ...
      rawMappingAliases: rawMapAliases,
      autoGeneratedAliases: autoGenAliases,
      effectiveAliases: effectiveAliases,
      optionRows,
      valueMap: parsedValueMap,
      reverseValueMap: parsedReverseValueMap,
      valueMapJson: valMapStr,
      reverseValueMapJson: revValMapStr
    };
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      const saved = updatedList.find(p => 
        (p.guid && editedProfile.guid && p.guid.toLowerCase().replace(/[^a-z0-9]/g, '') === editedProfile.guid.toLowerCase().replace(/[^a-z0-9]/g, '')) ||
        cleanGearNameForMatching(p.displayName) === cleanGearNameForMatching(editedProfile.displayName)
      );
      if (saved) {
        ...
        {isEditingParameter && paramForm && (
          <div className="fixed inset-0 bg-black/85 z-50 flex items-center justify-center p-4 backdrop-blur-sm overflow-y-auto">
            ...
            >
              <div className="flex justify-between items-center border-b border-white/5 pb-4">
                <div>
                  <h4 className="text-lg font-bold font-display uppercase tracking-wider text-white flex items-center gap-2">
                    <span className="w-2.5 h-2.5 bg-gear-accent rounded-full animate-pulse"></span>
                    AT5 Parameter Translation Editor
                  </h4>
                  <p className="text-[11px] text-gray-500 font-mono mt-1">Configure advanced translation rules, mapping coordinates, and visualization metadata</p>
                </div>
                <button
              ...
              <div className="space-y-6 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
                ...
                {/* GRID FOR GENERAL IDENTITY & STATUS */}
                <div className="border border-white/5 p-5 rounded-2xl bg-white/[0.01] space-y-4">
                  <span className="text-[11px] font-mono text-gear-accent uppercase font-bold tracking-wider block border-b border-white/5 pb-1.5">1. General Identity & Status Mapping</span>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        placeholder="e.g. Bass, Mid, Gain"
                      />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-mono text-gray-400 uppercase">Canonical Name</label>
                      <input
                        type="text"
                        className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-xs font-mono text-white focus:border-gear-accent/40"
                        value={paramForm.canonicalParameterName || paramForm.canonicalName || ''}
                        onChange={(e) => setParamForm({ ...paramForm, canonicalParameterName: e.target.value, canonicalName: e.target.value })}
                        placeholder="e.g. bass, mid, gain"
                      />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-mono text-gray-400 uppercase">AT5 Target XML Attribute ID</label>
                      <input
                        type="text"
                        className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-xs font-mono text-white focus:border-gear-accent/40"
                ...
                {/* PARAMETER ALIASES & MATCHING KEYS */}
                {(() => {
                  const savedAliases = getParamSavedAliases(paramForm);
                  const rawMapAliases = paramForm.rawMappingAliases || [];
                  const autoGenAliases = paramForm.autoGeneratedAliases && paramForm.autoGeneratedAliases.length > 0
                  ...
                  return (
                    <div className="border border-white/5 p-5 rounded-2xl bg-white/[0.01] space-y-4 w-full">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-white/5 pb-2">
                        <div>
                          <span className="text-[11px] font-mono text-gear-accent uppercase font-bold tracking-wider flex items-center gap-2">
                            ...
                            2. Parameter Aliases & Matching Keys
                          </span>
                          <p className="text-[10px] text-gray-400 mt-1">
                            Configure aliases for Tone Translator request-to-parameter matching. Gear-specific aliases take precedence over generic normalization.
                          </p>
                        </div>
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <button
                            ...
                            className="flex items-center gap-1 px-2.5 py-1 text-[10px] font-mono bg-white/5 hover:bg-white/10 text-gray-300 hover:text-white rounded-lg border border-white/10 transition-all cursor-pointer"
                            title="Remove duplicate aliases case-insensitively"
                          >
                          ...
                          <button
                            ...
                            className="flex items-center gap-1 px-2.5 py-1 text-[10px] font-mono bg-white/5 hover:bg-white/10 text-gray-300 hover:text-white rounded-lg border border-white/10 transition-all cursor-pointer"
                            title="Clean special characters and whitespace"
                          >
                      ...
                      {/* Bulk Paste Textarea Drawer */}
                      {showBulkParamAliasTextarea && (
                        <div className="p-4 bg-black/40 border border-white/10 rounded-xl space-y-3">
                          <div className="flex justify-between items-center">
                            <span className="text-[10px] font-mono uppercase font-bold text-gear-accent">Bulk Add Aliases (Comma or Line Separated)</span>
                            <span className="text-[9px] font-mono text-gray-500">Paste raw mapping strings or exported alias lists</span>
                          </div>
                          <textarea
                            rows={3}
                            className="w-full bg-white/5 border border-white/10 rounded-lg p-2.5 text-xs font-mono text-white focus:outline-none focus:border-gear-accent/40"
                            placeholder="e.g. Master, Volume_VintageMetalLead, Vol, MasterVolume&#10;gain, drive, Distortion"
                      ...
                      <div className="flex gap-2 items-center">
                        <div className="relative flex-1">
                          <input
                            type="text"
                            placeholder="Add single parameter alias (e.g. Master, Vol, Volume_VintageMetalLead)..."
                        ...
                        <button
                          ...
                          className="flex items-center gap-1 px-4 py-2 bg-gear-accent/15 hover:bg-gear-accent/25 text-gear-accent border border-gear-accent/30 text-xs font-mono font-bold uppercase rounded-xl transition-all cursor-pointer shrink-0"
                        >
                      ...
                      <div className="flex items-center gap-1 border-b border-white/5 pb-2 pt-1 overflow-x-auto text-[10px] font-mono">
                        ...
                        >
                          All ({savedAliases.length + rawMapAliases.length + autoGenAliases.length})
                        </button>
                        <button
                          type="button"
                          ...
                          className={`px-2.5 py-1 rounded-lg transition-all cursor-pointer ${
                            aliasGroupTab === 'saved'
                              ? 'bg-gear-accent/20 text-gear-accent font-bold border border-gear-accent/30'
                              : 'text-gray-400 hover:text-gray-200 hover:bg-white/5'
                          }`}
                        >
                        ...
                        <button
                          type="button"
                          onClick={() => setAliasGroupTab('effective')}
                          className={`px-2.5 py-1 rounded-lg transition-all cursor-pointer ${
                            aliasGroupTab === 'effective'
                              ? 'bg-emerald-500/20 text-emerald-300 font-bold border border-emerald-500/30'
                              : 'text-gray-400 hover:text-gray-200 hover:bg-white/5'
                          }`}
                        >
                          Effective Normalizer Set ({effectiveAliases.length})
                        </button>
                      </div>
                      ...
                      <div className="space-y-3 pt-1">
                        {/* SAVED ALIASES */}
                        {(aliasGroupTab === 'all' || aliasGroupTab === 'saved') && (
                          <div className="space-y-1.5">
                            <div className="flex justify-between items-center">
                              <span className="text-[10px] font-mono text-gear-accent uppercase font-bold tracking-wider">
                                Saved Aliases ({savedAliases.length})
                              </span>
                              <span className="text-[9px] font-mono text-gray-500">Explicitly edited for this gear parameter</span>
                            </div>
                            {savedAliases.length === 0 ? (
                              <p className="text-[11px] font-mono text-gray-500 italic py-1">No saved aliases defined.</p>
                            ) : (
                              <div className="flex flex-wrap gap-2">
                                {savedAliases.map((alias, idx) => (
                                  <div
                                    key={idx}
                                    className="flex items-center gap-1.5 bg-gear-accent/10 border border-gear-accent/25 rounded-lg px-2.5 py-1 text-xs font-mono text-white hover:border-gear-accent/50 transition-all"
                                  >
                                    {editingAliasIdx === idx ? (
                                      <input
                                        type="text"
                                        className="bg-black/60 border border-gear-accent/40 rounded px-1.5 py-0.5 text-xs font-mono text-white focus:outline-none"
                                        value={editingAliasVal}
                                        onChange={(e) => setEditingAliasVal(e.target.value)}
                                        onKeyDown={(e) => {
                                          if (e.key === 'Enter') handleUpdateParamSavedAlias(idx, editingAliasVal);
                                    ...
                                    {editingAliasIdx === idx ? (
                                      <button
                                        type="button"
                                        onClick={() => handleUpdateParamSavedAlias(idx, editingAliasVal)}
                        ...
                        {/* RAW MAPPINGS */}
                        {(aliasGroupTab === 'all' || aliasGroupTab === 'raw') && rawMapAliases.length > 0 && (
                          <div className="space-y-1.5 pt-2 border-t border-white/5">
                            <div className="flex justify-between items-center">
                              <span className="text-[10px] font-mono text-cyan-400 uppercase font-bold tracking-wider">
                                Raw Catalog Mappings ({rawMapAliases.length})
                              </span>
                              <span className="text-[9px] font-mono text-gray-500">Inherited from gear profile raw mapping catalog</span>
                            </div>
                            <div className="flex flex-wrap gap-2">
                              {rawMapAliases.map((alias, idx) => (
                                <div
                                  key={idx}
                                ...
                                >
                                  <span>{alias}</span>
                                  <span className="text-[8px] bg-cyan-500/20 px-1 rounded text-cyan-200">Raw</span>
                                </div>
                              ))}
                            </div>
                          </div>
                        ...
                        {/* AUTO-GENERATED */}
                        {(aliasGroupTab === 'all' || aliasGroupTab === 'auto') && autoGenAliases.length > 0 && (
                          <div className="space-y-1.5 pt-2 border-t border-white/5">
                            <div className="flex justify-between items-center">
                              <span className="text-[10px] font-mono text-purple-400 uppercase font-bold tracking-wider">
                                Auto-Generated Variations ({autoGenAliases.length})
                              </span>
                              <span className="text-[9px] font-mono text-gray-500">Derived algorithmically from parameter & gear name</span>
                            </div>
                            <div className="flex flex-wrap gap-2">
                              {autoGenAliases.map((alias, idx) => (
                                <div
                                  key={idx}
                                  className="flex items-center gap-1.5 bg-purple-500/10 border border-purple-500/20 rounded-lg px-2.5 py-1 text-xs font-mono text-purple-300"
                                >
                                  <span>{alias}</span>
                                  <span className="text-[8px] bg-purple-500/20 px-1 rounded text-purple-200">Auto</span>
                                </div>
                              ))}
                            </div>
                          </div>
                        ...
                        {/* EFFECTIVE RESOLUTION SET */}
                        {(aliasGroupTab === 'all' || aliasGroupTab === 'effective') && (
                          <div className="space-y-1.5 pt-2 border-t border-white/5">
                            <div className="flex justify-between items-center">
                              <span className="text-[10px] font-mono text-emerald-400 uppercase font-bold tracking-wider">
                                Effective Resolution Set ({effectiveAliases.length})
                              </span>
                              <span className="text-[9px] font-mono text-gray-500">Active keys matched by normalizer for this gear item</span>
                            </div>
                            <div className="flex flex-wrap gap-2">
                              {effectiveAliases.map((alias, idx) => (
                                <div
                                  key={idx}
                                  className="flex items-center gap-1 bg-emerald-500/10 border border-emerald-500/20 rounded-lg px-2.5 py-1 text-xs font-mono text-emerald-300"
                                >
                ...
                {/* GRID FOR PRESENTATION & INTERFACE */}
                <div className="border border-white/5 p-5 rounded-2xl bg-white/[0.01] space-y-4 w-full">
                  <span className="text-[11px] font-mono text-gear-accent uppercase font-bold tracking-wider block border-b border-white/5 pb-1.5">3. Interface Type & Range Boundaries</span>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="space-y-1.5">
                      ...
                      >
                        <option value="knob">Continuous Knob</option>
                        ...
                        <option value="hidden_default">Hidden/Default Background</option>
                      </select>
                    </div>
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-mono text-gray-400 uppercase">Decimal Precision</label>
                      <input
                        type="number"
                        className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-xs font-mono text-white focus:border-gear-accent/40"
                        value={paramForm.decimalPlaces !== undefined ? paramForm.decimalPlaces : ''}
                        onChange={(e) => setParamForm({ ...paramForm, decimalPlaces: e.target.value !== '' ? Number(e.target.value) : undefined })}
                        placeholder="e.g. 1, 2"
                      />
                    </div>

                    <div className="space-y-1.5">
                      <label className="text-[10px] font-mono text-gray-400 uppercase">Default Display Value</label>
                      <input
                        type="text"
                        className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-xs font-mono text-white focus:border-gear-accent/40"
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-6 gap-3 pt-2">
                    <div className="space-y-1">
                      <span className="text-[9px] font-mono text-gray-400 uppercase">UI Visual Min</span>
                      <input
                        type="number"
                    ...
                    <div className="space-y-1">
                      <span className="text-[9px] font-mono text-gray-400 uppercase">UI Visual Max</span>
                      <input
                        type="number"
                        className="w-full bg-white/5 border border-white/10 rounded-lg px-2.5 py-1.5 text-xs font-mono text-white focus:border-gear-accent/40"
                    ...
                    <div className="space-y-1 col-span-2">
                      <span className="text-[9px] font-mono text-gray-400 uppercase">UI Visual Unit</span>
                      <input
                        type="text"
                        className="w-full bg-white/5 border border-white/10 rounded-lg px-2.5 py-1.5 text-xs font-mono text-white focus:border-gear-accent/40"
                        ...
                        placeholder="dB, Hz, ms, %"
                      />
                    </div>
                    <div className="space-y-1">
                      <span className="text-[9px] font-mono text-gray-400 uppercase">Display Min</span>
                      <input
                        type="number"
                        className="w-full bg-white/5 border border-white/10 rounded-lg px-2.5 py-1.5 text-xs font-mono text-white focus:border-gear-accent/40"
                        value={paramForm.displayMin !== undefined ? paramForm.displayMin : ''}
                        onChange={(e) => setParamForm({ ...paramForm, displayMin: e.target.value !== '' ? Number(e.target.value) : undefined })}
                      />
                    </div>
                    <div className="space-y-1">
                      <span className="text-[9px] font-mono text-gray-400 uppercase">Display Max</span>
                      <input
                        type="number"
                        className="w-full bg-white/5 border border-white/10 rounded-lg px-2.5 py-1.5 text-xs font-mono text-white focus:border-gear-accent/40"
                        value={paramForm.displayMax !== undefined ? paramForm.displayMax : ''}
                        onChange={(e) => setParamForm({ ...paramForm, displayMax: e.target.value !== '' ? Number(e.target.value) : undefined })}
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="border border-white/5 p-5 rounded-2xl bg-white/[0.01] space-y-4 w-full">
                  <span className="text-[11px] font-mono text-gear-accent uppercase font-bold tracking-wider block border-b border-white/5 pb-1.5">4. Translation Strategy & Formulas</span>
                  <div className="grid grid-cols-1 gap-4 w-full">
                    <div className="space-y-1.5 w-full">
                      ...
                      >
                        <option value="direct">direct (Linearly Mapped)</option>
                        <option value="dbThresholdToLinear">dbThresholdToLinear (Dynamic Thresholds)</option>
                        <option value="db_to_linear">db_to_linear (Decibels to Float)</option>
                        ...
                        <option value="scaled_range">scaled_range (Custom Ranges)</option>
                        <option value="enum">enum (Indexed Dropdowns / Selectors)</option>
                    ...
                    <div className="space-y-1.5 w-full">
                      <div className="flex justify-between items-center">
                        ...
                        <label className="flex items-center gap-1.5 text-[9px] font-mono text-gray-400 hover:text-white cursor-pointer select-none">
                          <input
                            type="checkbox"
                            className="w-3 h-3 rounded border-white/10 bg-white/5 text-gear-accent focus:ring-0 focus:ring-offset-0 cursor-pointer"
                      ...
                      <input
                        ...
                        placeholder="e.g. y = x / 10, or decibel mapping scale"
                      />
                    </div>
                  </div>

                  {/* STRUCTURED OPTIONS & ENUMS EDITOR */}
                  <div className="border border-white/5 p-5 rounded-2xl bg-white/[0.01] space-y-4">
                    <div className="flex justify-between items-center border-b border-white/5 pb-2">
                      <div>
                        <span className="text-[11px] font-mono text-gear-accent uppercase font-bold tracking-wider flex items-center gap-2">
                          <Layers className="w-3.5 h-3.5" />
                          Structured Options & Selector Values
                        </span>
                        <p className="text-[10px] text-gray-400 mt-1">
                          Configure rotary selector positions, enum mappings, or switch states.
                        </p>
                      </div>
                      <button
                        type="button"
                        onClick={handleAddOptionRow}
                        className="flex items-center gap-1 px-2.5 py-1 text-[10px] font-mono bg-gear-accent/10 hover:bg-gear-accent/20 text-gear-accent rounded-lg border border-gear-accent/20 transition-all cursor-pointer"
                      >
                    ...
                    {optionRows.length === 0 ? (
                      <div className="py-6 text-center border border-dashed border-white/5 rounded-xl bg-white/[0.01]">
                        <span className="text-[11px] font-mono text-gray-500">No structured options defined. Convert from JSON or click "Add Option" to start.</span>
                      </div>
                    ) : (
                      <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                        ...
                        <div className="space-y-2">
                          {optionRows.map((row, rIdx) => {
                            // Validation checks
                            const hasDuplicateLabel = optionRows.some((r, idx) => idx !== rIdx && r.displayLabel && r.displayLabel.trim().toLowerCase() === row.displayLabel.trim().toLowerCase());
                            const hasDuplicateExport = optionRows.some((r, idx) => idx !== rIdx && r.exportValue !== undefined && String(r.exportValue).trim().toLowerCase() === String(row.exportValue).trim().toLowerCase());
                            ...
                            return (
                              <div
                                ...
                                className={`grid grid-cols-12 gap-2 items-center bg-white/[0.02] p-2 rounded-xl border transition-all ${
                                  isEmptyLabel || isEmptyExport
                                    ? 'border-red-500/20'
                                    : hasDuplicateLabel || hasDuplicateExport
                              ...
                              >
                                ...
                                <div className="col-span-3 relative">
                                  <input
                                    type="text"
                                    className={`w-full bg-white/5 border rounded-lg px-2 py-1 text-xs font-mono text-white focus:outline-none focus:border-gear-accent/40 ${
                                      isEmptyLabel ? 'border-red-500/30' : 'border-white/5'
                                    }`}
                                    value={row.displayLabel}
                                    onChange={(e) => handleUpdateOptionRow(rIdx, 'displayLabel', e.target.value)}
                                    placeholder="e.g. ST, Off, Chorus"
                                  />
                                  {isEmptyLabel && (
                                    <span className="absolute -bottom-3 left-1 text-[8px] font-mono text-red-400">Required</span>
                                  )}
                                  {hasDuplicateLabel && (
                                    <span className="absolute -bottom-3 left-1 text-[8px] font-mono text-yellow-400">Duplicate</span>
                                  )}
                                </div>
                                ...
                                <div className="col-span-3">
                                  <input
                                    type="text"
                                    className="w-full bg-white/5 border border-white/5 rounded-lg px-2 py-1 text-xs font-mono text-gray-300 focus:outline-none focus:border-gear-accent/40"
                                    value={row.aliases.join(', ')}
                                    onChange={(e) => {
                                      const arr = e.target.value.split(',').map(s => s.trim()).filter(Boolean);
                                      handleUpdateOptionRow(rIdx, 'aliases', arr);
                                    }}
                                    placeholder="e.g. stereo, bypass"
                                  />
                                </div>
                                ...
                                <div className="col-span-3 relative">
                                  <input
                                    type="text"
                                    className={`w-full bg-white/5 border rounded-lg px-2 py-1 text-xs font-mono text-white focus:outline-none focus:border-gear-accent/40 ${
                                      isEmptyExport ? 'border-red-500/30' : 'border-white/5'
                                    }`}
                                    value={row.exportValue !== undefined ? String(row.exportValue) : ''}
                                    onChange={(e) => handleUpdateOptionRow(rIdx, 'exportValue', e.target.value)}
                                    placeholder="e.g. ST, 0.0, LCR"
                                  />
                                  {isEmptyExport && (
                                    <span className="absolute -bottom-3 left-1 text-[8px] font-mono text-red-400">Required</span>
                                  )}
                                  {hasDuplicateExport && (
                                    <span className="absolute -bottom-3 left-1 text-[8px] font-mono text-yellow-400">Duplicate export</span>
                                  )}
                                </div>
                                ...
                                <div className="col-span-1 flex justify-center">
                                  <input
                                    type="checkbox"
                                    className="w-3.5 h-3.5 rounded border-white/10 bg-white/5 text-gear-accent focus:ring-0 focus:ring-offset-0 cursor-pointer"
                                ...
                                {/* Notes */}
                                <div className="col-span-2 relative flex items-center gap-1.5">
                                  <input
                                    type="text"
                                    className="w-full bg-white/5 border border-white/5 rounded-lg px-2 py-1 text-[10px] font-mono text-gray-400 focus:outline-none focus:border-gear-accent/40"
                                    value={row.notes || ''}
                                  ...
                                  <button
                                    ...
                                    className="p-1 text-gray-500 hover:text-red-400 hover:bg-red-500/10 rounded transition-all cursor-pointer shrink-0"
                                    title="Delete row"
                                  >
                  ...
                  {/* ADVANCED COLLAPSIBLE JSON PANEL */}
                  <div className="border border-white/5 rounded-2xl bg-white/[0.005] overflow-hidden">
                    <details className="group">
                      <summary className="flex justify-between items-center px-5 py-3 text-[11px] font-mono text-gray-400 hover:text-white uppercase font-bold tracking-wider cursor-pointer select-none bg-white/[0.01] hover:bg-white/[0.02] transition-all">
                        <span className="flex items-center gap-2">
                          <FileCode className="w-3.5 h-3.5" />
                          Advanced Raw JSON Maps
                        </span>
                        <span className="text-[10px] font-normal normal-case text-gray-500 group-open:hidden">Show raw editor</span>
                        <span className="text-[10px] font-normal normal-case text-gray-500 hidden group-open:block">Hide raw editor</span>
                      </summary>
                      
                      <div className="p-5 space-y-4 border-t border-white/5 bg-black/20">
                        <p className="text-[10px] text-gray-400">
                          These JSON maps are automatically kept in sync with your structured option rows above on save. You can also manually sync from edited JSON.
                        </p>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-1.5">
                            ...
                            <textarea
                              className={`w-full h-24 bg-white/5 border rounded-xl p-3 text-xs font-mono text-gray-300 focus:outline-none focus:border-gear-accent/40 ${
                                valueMapError ? 'border-red-500/50 focus:border-red-500' : 'border-white/10'
                              }`}
                              value={valueMapText}
                              onChange={(e) => {
                          ...
                          <div className="space-y-1.5">
                            ...
                            <textarea
                              className={`w-full h-24 bg-white/5 border rounded-xl p-3 text-xs font-mono text-gray-300 focus:outline-none focus:border-gear-accent/40 ${
                                reverseValueMapError ? 'border-red-500/50 focus:border-red-500' : 'border-white/10'
                              }`}
                ...
                {/* EXPORT CONSTRAINTS */}
                <div className="border border-white/5 p-5 rounded-2xl bg-white/[0.01] space-y-4 w-full">
                  <span className="text-[11px] font-mono text-gear-accent uppercase font-bold tracking-wider block border-b border-white/5 pb-1.5">5. Advanced Export Constraints</span>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-mono text-gray-400 uppercase">AmpliTube Float Min</label>
                      <input
                        type="number"
                        step="0.0001"
                        className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-xs font-mono text-white focus:border-gear-accent/40"
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-mono text-gray-400 uppercase">AmpliTube Float Max</label>
                      <input
                        type="number"
                        step="0.0001"
                        className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-xs font-mono text-white focus:border-gear-accent/40"
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-mono text-gray-400 uppercase">Export Step Increment</label>
                      <input
                        type="number"
                        step="0.0001"
                        className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-xs font-mono text-white focus:border-gear-accent/40"
                        value={paramForm.exportStep !== undefined ? paramForm.exportStep : ''}
                        onChange={(e) => setParamForm({ ...paramForm, exportStep: e.target.value !== '' ? Number(e.target.value) : undefined })}
                        placeholder="e.g. 0.01, 0.1"
                      />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-mono text-gray-400 uppercase">Default Export Value</label>
                      <input
                        type="text"
                        className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-xs font-mono text-white focus:border-gear-accent/40"
                ...
                {/* HELP, DOCUMENTATION & EXCEPTIONS */}
                <div className="border border-white/5 p-5 rounded-2xl bg-white/[0.01] space-y-4 w-full">
                  <span className="text-[11px] font-mono text-gear-accent uppercase font-bold tracking-wider block border-b border-white/5 pb-1.5">6. Quality Control & Documentation</span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        placeholder="What does this parameter do in plain English?"
                      />
                    </div>
                    <div className="space-y-1.5">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        placeholder="e.g. 0.55, 0.0, 1.0"
                      />
                    </div>
                  </div>

                  {/* CHECKBOX & REASON FOR DO NOT REQUEST */}
                  <div className="border border-white/5 p-4 rounded-xl bg-black/40 space-y-3">
                    <label className="flex items-center gap-2.5 cursor-pointer select-none">
                      <input
                        type="checkbox"
                        className="w-4 h-4 rounded border-white/10 bg-white/5 text-gear-accent focus:ring-0 focus:ring-offset-0"
                      ...
                      <div>
                        ...
                        <span className="text-[10px] text-gray-400 font-mono block">Check this if the gear item does not support or shouldn't attempt to map this parameter</span>
                      </div>
                    </label>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-1">
                        <div className="space-y-1">
                          ...
                          <input
                            ...
                            placeholder="e.g. This booster has no Tone knob"
                          />
                        </div>
                        <div className="space-y-1">
