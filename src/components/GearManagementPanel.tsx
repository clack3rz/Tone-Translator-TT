...
export const GearManagementPanel: React.FC<GearManagementPanelProps> = ({ onRefreshChain, onClose, initialSelectedGuid, exportDebugData }) => {
  ...
  const [aliasGroupTab, setAliasGroupTab] = useState<'all' | 'saved' | 'raw_map' | 'auto_gen'>('all');
  ...
  const handleStartEditParam = (index: number) => {
    ...
    if (rawParam) {
      ...
      setParamForm({
        ...
        displayParameterName: rawParam.displayParameterName || rawParam.displayName || rawParam.canonicalName || '',
        canonicalName: rawParam.canonicalName || rawParam.canonicalParameterName || rawParam.export?.name || '',
        canonicalParameterName: rawParam.canonicalParameterName || rawParam.canonicalName || rawParam.export?.name || '',
        savedAliases: saved,
        aliases: saved,
        ...
        displayPrecision,
        displayDecimalPlaces: displayPrecision,
        decimalPlaces: displayPrecision,
        exportPrecision,
        exportDecimalPlaces: exportPrecision,
        displayMin: rawParam.displayMin ?? rawParam.visualMin ?? rawParam.visual?.min ?? 0,
        displayMax: rawParam.displayMax ?? rawParam.visualMax ?? rawParam.visual?.max ?? 10,
        displayUnit: rawParam.displayUnit ?? rawParam.visual?.unit ?? '',
        visual: {
          min: rawParam.visual?.min ?? rawParam.visualMin ?? 0,
          max: rawParam.visual?.max ?? rawParam.visualMax ?? 10,
          unit: rawParam.visual?.unit ?? rawParam.unit ?? ''
        },
        export: {
          name: rawParam.export?.name ?? rawParam.at5XmlAttributeName ?? rawParam.canonicalName ?? '',
          min: rawParam.export?.min ?? rawParam.exportMin ?? rawParam.min ?? 0,
          max: rawParam.export?.max ?? rawParam.exportMax ?? rawParam.max ?? 1
        },
        conversion: {
          mode: rawParam.conversion?.mode ?? rawParam.translationMode ?? 'direct',
  ...
  const handleSaveParamEdit = async () => {
    ...
    const finalForm: GearProfileParameter = {
      ...
      displayParameterName: paramForm.displayParameterName || paramForm.displayName || paramForm.canonicalName,
      canonicalName: paramForm.canonicalName || paramForm.canonicalParameterName || paramForm.export?.name || '',
      canonicalParameterName: paramForm.canonicalParameterName || paramForm.canonicalName || paramForm.export?.name || '',
      displayPrecision: dispPrec,
      displayDecimalPlaces: dispPrec,
      decimalPlaces: dispPrec,
      exportPrecision: expPrec,
      exportDecimalPlaces: expPrec,
      ...
      visual: {
        min: paramForm.visual?.min !== undefined ? Number(paramForm.visual.min) : 0,
        max: paramForm.visual?.max !== undefined ? Number(paramForm.visual.max) : 10,
        unit: paramForm.visual?.unit || ''
      },
    ...
    if (!user) {
      try {
        ...
        setParamSaveError("Please sign in to persist gear parameters to the database.");
        return;
      }
    }
    ...
    try {
      ...
      const savedProfile = reloadedProfiles.find(p => 
        (updatedProfile.id && p.id === updatedProfile.id) ||
      ...
      setImportFeedback(`Parameter "${finalForm.displayName}" saved and persisted successfully.`);
      ...
      console.error("Parameter save failed:", err);
  ...
  return (
    <div className="bg-[#0e0e11] border border-white/5 rounded-3xl p-6 md:p-8 space-y-8 text-white relative overflow-hidden">
      ...
      <ParameterTranslationEditorModal
        ...
        isSaving={isSavingParamEdit}
        saveError={paramSaveError}
        optionRows={optionRows}
        setOptionRows={setOptionRows}
        valueMapText={valueMapText}
        setValueMapText={setValueMapText}
        reverseValueMapText={reverseValueMapText}
        setReverseValueMapText={setReverseValueMapText}
        valueMapError={valueMapError}
        setValueMapError={setValueMapError}
        reverseValueMapError={reverseValueMapError}
        setReverseValueMapError={setReverseValueMapError}
        lockFormulaDesc={lockFormulaDesc}
        setLockFormulaDesc={setLockFormulaDesc}
        showBulkParamAliasTextarea={showBulkParamAliasTextarea}
        setShowBulkParamAliasTextarea={setShowBulkParamAliasTextarea}
        bulkParamAliasText={bulkParamAliasText}
        setBulkParamAliasText={setBulkParamAliasText}
        newParamAliasInput={newParamAliasInput}
        setNewParamAliasInput={setNewParamAliasInput}
        editingAliasIdx={editingAliasIdx}
        setEditingAliasIdx={setEditingAliasIdx}
        editingAliasVal={editingAliasVal}
        setEditingAliasVal={setEditingAliasVal}
        aliasGroupTab={aliasGroupTab}
        setAliasGroupTab={setAliasGroupTab}
        paramAliasError={paramAliasError}
        setParamAliasError={setParamAliasError}
        testInputValue={testInputValue}
        setTestInputValue={setTestInputValue}
        testTranslationResult={testTranslationResult}
        handleAddOptionRow={handleAddOptionRow}
        handleUpdateOptionRow={handleUpdateOptionRow}
        handleSetDefaultRow={handleSetDefaultRow}
        handleRemoveOptionRow={handleRemoveOptionRow}
        handleSyncFromRawJSON={handleSyncFromRawJSON}
        handleAddParamSavedAlias={handleAddParamSavedAlias}
        handleUpdateParamSavedAlias={handleUpdateParamSavedAlias}
        handleRemoveParamSavedAlias={handleRemoveParamSavedAlias}
        handleApplyBulkParamAliases={handleApplyBulkParamAliases}
        handleDeduplicateParamAliases={handleDeduplicateParamAliases}
        handleNormalizeParamAliases={handleNormalizeParamAliases}
        checkParamAliasCollisions={checkParamAliasCollisions}
        getParamSavedAliases={getParamSavedAliases}
        generateAliasesForXmlParam={generateAliasesForXmlParam}
      />
