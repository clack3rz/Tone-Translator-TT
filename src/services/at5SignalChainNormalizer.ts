...
const normaliseCompressorSettings = (
  ...
  for (const [key, value] of Object.entries(settings)) {
    ...
    else if (k === "output" || k === "volume") out["Output"] = value;
    else if (k === "level") out["Level"] = value;
    else if (k === "sensitivity" || k === "sens" || k === "comp" || k === "compression") out["Comp"] = value;
    else out[key] = value;
  }
...
const normaliseSettings = (
  ...
  let gearParams: ReturnType<typeof resolveGearParameters> = [];
  try {
    gearParams = resolveGearParameters(canonicalName, gearType);
    if (!gearParams || gearParams.length === 0) {
      gearParams = resolveGearParameters(gearName, gearType);
    }
  } catch {
    gearParams = [];
  }

  const gearMatchedSettings: Record<string, string | number> = {};
  const remainingSettings: Record<string, any> = {};

  if (gearParams && gearParams.length > 0) {
    for (const [key, value] of Object.entries(settings)) {
      const cleanKey = key.toLowerCase().replace(/[^a-z0-9.]/g, "");
      const matchedParam = gearParams.find(p => {
        const cleanFriendly = p.friendlyName.toLowerCase().replace(/[^a-z0-9.]/g, "");
        const cleanXml = p.xmlName.toLowerCase().replace(/[^a-z0-9.]/g, "");
        const cleanCanonical = (p.canonicalParameterName || "").toLowerCase().replace(/[^a-z0-9.]/g, "");
        const cleanDisplay = (p.displayParameterName || "").toLowerCase().replace(/[^a-z0-9.]/g, "");
        const cleanAliases = [
          ...(p.aliases || []),
          ...(p.savedAliases || []),
          ...(p.rawMappingAliases || []),
          ...(p.autoGeneratedAliases || []),
          ...(p.effectiveAliases || [])
        ...
        return (
          ...
          cleanXml === cleanKey ||
          (cleanCanonical && cleanCanonical === cleanKey) ||
          (cleanDisplay && cleanDisplay === cleanKey) ||
          cleanAliases.includes(cleanKey)
        );
      });

      if (matchedParam) {
        gearMatchedSettings[matchedParam.friendlyName] = value;
      } else {
        remainingSettings[key] = value;
      }
    }
  } else {
    Object.assign(remainingSettings, settings);
  }

  if (Object.keys(remainingSettings).length === 0) {
    return gearMatchedSettings;
  }

  let normalizedRemaining: Record<string, string | number> = remainingSettings;

  if (canonicalName === "Noise Gate") {
    normalizedRemaining = normaliseNoiseGateSettings(remainingSettings);
  } else if (canonicalName === "OverScream") {
    normalizedRemaining = normaliseOverScreamSettings(remainingSettings);
  } else if (gearType === "amp") {
    normalizedRemaining = normaliseAmpSettings(remainingSettings, canonicalName);
  } else if (gearType === "cab") {
    normalizedRemaining = normaliseCabSettings(remainingSettings);
  } else if (n === "parametric eq" || n === "parametric eq 3") {
    normalizedRemaining = normaliseParametricEqSettings(remainingSettings);
  } else if (n.includes("graphic") || n === "eq pg") {
    normalizedRemaining = normaliseGraphicEqSettings(remainingSettings);
  } else if (
    n.includes("compressor") ||
    n.includes("comp") ||
    n.includes("black 76")
  ) {
    normalizedRemaining = normaliseCompressorSettings(remainingSettings);
  }
