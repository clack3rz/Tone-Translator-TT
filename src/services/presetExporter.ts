...
import {
  ...
  resolveSelectorTargetForParam,
  getSelectorMappingDebugInfo,
...
export interface ExportDebugItem {
  ...
  selector_value?: string | number;
  active_bank?: string;
  bank_mappings?: Record<string, string>;
  selector_mapping_status?: "SUCCESS" | "UNRESOLVED" | "INFERRED" | "DEFAULT_USED";
  selector_mapping_reason?: string;
...
const makeDebugItem = (
  ...
  if (exported && group !== "cab") {
    ...
    if (defs && defs.length > 0) {
      for (const [normKey, normVal] of Object.entries(normSettings)) {
        ...
        let isSelectorBankMapping = false;
        let selectorBankNote = "";
        if (!def) {
          const selectorTarget = resolveSelectorTargetForParam(gear.name, group, normKey, gear.settings, defs);
          if (selectorTarget) {
            def = selectorTarget.targetDef;
            isSelectorBankMapping = true;
            selectorBankNote = `Mapped via selector ${selectorTarget.selectorParam} (${selectorTarget.activeBankLabel} active) to ${def.xmlName}`;
          }
        }
        ...
        if (def) {
          ...
          if (expVal !== undefined) {
            ...
            if (isSelectorBankMapping) {
              match_source = "exact_xml";
              conversion_note = selectorBankNote;
            } else if (cleanXml === cleanNormKey) {
              match_source = "exact_xml";
    ...
    const exported_value_0 = {
      Mic0Angle: parsedExported.Mic0Angle !== undefined && parsedExported.Mic0Angle !== null ? parsedExported.Mic0Angle : "0",
      Mic0XAxis: parsedExported.Mic0XAxis !== undefined && parsedExported.Mic0XAxis !== null ? parsedExported.Mic0XAxis : "0",
      Mic0YAxis: parsedExported.Mic0YAxis !== undefined && parsedExported.Mic0YAxis !== null ? parsedExported.Mic0YAxis : "0",
      Mic0Distance: parsedExported.Mic0Distance !== undefined && parsedExported.Mic0Distance !== null ? parsedExported.Mic0Distance : "0",
      Mic0Speaker: parsedExported.Mic0Speaker !== undefined && parsedExported.Mic0Speaker !== null ? parsedExported.Mic0Speaker : "0"
    };
    ...
    if (!was_supplied_0) {
      ...
      for (const [f, expectedVal] of Object.entries(xmlValues0)) {
        if (f.startsWith("Mic0")) {
          const actualVal = parsedExported[f] !== undefined && parsedExported[f] !== null ? parsedExported[f] : "0";
          const evNum = parseFloat(String(expectedVal));
          const avNum = parseFloat(String(actualVal));
          let matched = false;
          if (!isNaN(avNum) && !isNaN(evNum)) {
            matched = Math.abs(avNum - evNum) <= 0.0001;
          } else {
    ...
    const exported_value_1 = {
      Mic1Angle: parsedExported.Mic1Angle !== undefined && parsedExported.Mic1Angle !== null ? parsedExported.Mic1Angle : "0",
      Mic1XAxis: parsedExported.Mic1XAxis !== undefined && parsedExported.Mic1XAxis !== null ? parsedExported.Mic1XAxis : "0",
      Mic1YAxis: parsedExported.Mic1YAxis !== undefined && parsedExported.Mic1YAxis !== null ? parsedExported.Mic1YAxis : "0",
      Mic1Distance: parsedExported.Mic1Distance !== undefined && parsedExported.Mic1Distance !== null ? parsedExported.Mic1Distance : "0",
      Mic1Speaker: parsedExported.Mic1Speaker !== undefined && parsedExported.Mic1Speaker !== null ? parsedExported.Mic1Speaker : "1"
    };
    ...
    const micKeysToSkip = new Set([
      "speaker", "room",
      // Canonical AT5 Mic 0
      "mic_0", "mic 0", "mic0", "mic_0_placement", "mic 0 placement", "mic0_placement", "mic0 placement",
      ...
      "mic_0_level", "mic 0 level", "mic0_level", "mic0 level",
      "position_0", "position 0", "position0",
      "distance_0", "distance 0", "distance0",
      "angle_0", "angle 0", "angle0",
      "axis_0", "axis 0", "axis0",
      // Canonical AT5 / Legacy Mic 1
      "mic_1", "mic 1", "mic1", "mic a", "mic_1_placement", "mic 1 placement", "mic1_placement", "mic1 placement",
      ...
      "mic_1_level", "mic 1 level", "mic1_level", "mic1 level",
      "position_1", "position 1", "position1",
      "distance_1", "distance 1", "distance1",
      "angle_1", "angle 1", "angle1",
      "axis_1", "axis 1", "axis1",
      // Legacy Mic 2
      "mic_2", "mic 2", "mic2", "mic b", "mic_2_placement", "mic 2 placement", "mic2_placement", "mic2 placement",
      ...
      "mic_2_level", "mic 2 level", "mic2_level", "mic2 level",
      "position_2", "position 2", "position2",
      "distance_2", "distance 2", "distance2",
      "angle_2", "angle 2", "angle2",
      "axis_2", "axis 2", "axis2",
      // Generic
      "position", "distance", "angle", "axis", "mic_placement", "mic placement", "placement"
    ]);
  ...
  let selector_parameter: string | undefined = undefined;
  let selector_value: string | number | undefined = undefined;
  let active_bank: string | undefined = undefined;
  let bank_mappings: Record<string, string> | undefined = undefined;
  let selector_mapping_status: "SUCCESS" | "UNRESOLVED" | "INFERRED" | "DEFAULT_USED" | undefined = undefined;
  let selector_mapping_reason: string | undefined = undefined;

  const defs = getParameterDefinitions(gear.name, group);
  const selectorInfo = getSelectorMappingDebugInfo(gear.name, group, gear.settings, defs, parsedExported, dropped_parameters);
  if (selectorInfo) {
    selector_parameter = selectorInfo.selectorParameter;
    selector_value = selectorInfo.selectorValue;
    active_bank = selectorInfo.activeBank;
    bank_mappings = selectorInfo.bankMappings;
    selector_mapping_status = selectorInfo.status;
    selector_mapping_reason = selectorInfo.reason;

    if (selectorInfo.status === "SUCCESS") {
      if (verified_guid_resolved) {
    ...
    } else if (selectorInfo.status === "UNRESOLVED") {
      final_status = "FAIL";
      finalReason = selectorInfo.reason;
    }

    if (gear.name === "Darrell 100") {
      darrell_channel_selected = String(selector_value);
      darrell_active_gain_parameter = bank_mappings["Gain"] || bank_mappings["Pre Amp"];
      darrell_active_master_parameter = bank_mappings["Master"] || bank_mappings["Volume"];
      darrell_channel_mapping_confidence = "verified_at5p";
      darrell_channel_mapping_reason = selector_mapping_reason;
    }
  }
  ...
  return {
    ...
    active_bank,
    bank_mappings,
    selector_mapping_status,
    selector_mapping_reason,
    ...
    element_name: originalRequestedGearName || gear.name,
    details: detailsList,
              ...
              if (actualVal === undefined) {
                ...
                if (expectedVal !== undefined) {
                  ...
                  if (!isNaN(evNum) && !isNaN(avNum)) {
                    match = Math.abs(evNum - avNum) <= 0.0001;
                  } else {
