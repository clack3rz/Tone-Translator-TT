...
export interface MicPlacementDecision {
  ...
  distance: SemanticDistance;
  angle: SemanticAngle;
  placementString: string; // e.g. "Cap Edge, W, Close, On Axis" or "Cap, Close, On Axis"
...
export function reasonSemanticMicPlacement(input: MicReasoningInput): MicPlacementDecision {
  ...
  if (existingPlacement && isCompleteSemanticPlacement(existingPlacement)) {
    ...
    if (parsed.position && parsed.distance && parsed.angle) {
      ...
      } else if (parsed.hasExplicitOrientation && parsed.orientation) {
        orientation = parsed.orientation;
      } else {
        // Legacy 3-part placement preserved without synthetic orientation injection
        orientation = undefined;
      }
  ...
  const placementString = formatSemanticPlacement(position, distance, angle, orientation);
...
export function formatMicrophoneDebug(decisions: MicPlacementDecision[]): string {
  ...
  return decisions.map(d => {
    ...
    const placementDisplay = d.position === "Cap" || !d.orientation
      ? `${d.position} / ${d.distance} / ${d.angle}`
      : `${d.position} / ${d.orientation} / ${d.distance} / ${d.angle}`;
    ...
    if (el.type === "cab" || (el as any).type === "cabinet") {
      ...
      if (isLegacy) {
        ...
        if (mic1Name) {
          ...
          if (m1Decision.orientation) {
            settings["Mic_1_Orientation"] = m1Decision.orientation;
          }
        ...
        if (mic2Name) {
          ...
          const m2Decision = reasonSemanticMicPlacement({
            ...
            otherMic: m1Decision ? {
              ...
              orientation: m1Decision.orientation,
          ...
          if (m2Decision.orientation) {
            settings["Mic_2_Orientation"] = m2Decision.orientation;
          }
        ...
        if (mic0Name) {
          ...
          if (m0Decision.orientation) {
            settings["Mic_0_Orientation"] = m0Decision.orientation;
          }
        ...
        if (mic1Name) {
          ...
          const m1Decision = reasonSemanticMicPlacement({
            ...
            otherMic: m0Decision ? {
              ...
              orientation: m0Decision.orientation,
