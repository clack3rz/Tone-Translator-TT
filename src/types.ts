...
export interface ParameterMapping {
  ...
  // Extended ParameterTranslation model fields
  gearGuid?: string;
  ...
  validationStatus?: 'PASS' | 'CHECK' | 'WARN' | 'FAIL' | 'UNSUPPORTED' | 'NEEDS_MAPPING' | 'VERIFIED_AT5P';
...
export interface RequestedParameterReview {
  ...
  status: 'CHECK' | 'ADD_MAPPING' | 'MAP_TO_EXISTING' | 'MARK_UNSUPPORTED' | 'IGNORE_ONCE' | 'DO_NOT_REQUEST';
  notes?: string;
  updatedAt?: any;
  updatedBy?: string;
}
...
export interface GearProfileParameter {
  ...
  validationStatus: 'PASS' | 'CHECK' | 'WARN' | 'PARTIAL' | 'FAIL' | 'UNKNOWN' | 'UNSUPPORTED' | 'NEEDS_MAPPING' | 'VERIFIED_AT5P';

  // Extended fields mirroring ParameterMapping
  gearGuid?: string;
  displayParameterName?: string;
  canonicalParameterName?: string;
  savedAliases?: string[];
